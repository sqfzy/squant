...
use crate::{
    ...
    data::{BookData, CandleData, DataStream},
};
...
impl OkxClientV5 {
    #[builder]
    pub fn new(
        #[builder(default = "http://www.okx.com/")] base_http_url: &str,
        #[builder(default = "ws://wspap.okx.com/")] base_ws_uri: &str,
    ) -> Result<Self> {
...
impl DataSubscriber<OkxWsResponse<OkxCandleData>> for OkxClientV5 {
    async fn subscribe_data(
        &mut self,
        params: <OkxWsResponse<OkxCandleData> as DataResponse>::Request,
    ) -> Result<impl StreamExt<Item = <OkxWsResponse<OkxCandleData> as RawData>::Data> + Send> {
        ...
        {
            simd_json::serde::from_slice::<OkxWsResponse<OkxCandleData>>(
        ...
        let stream = DataStream::new(client, |msg| -> Result<_> {
            let msg = msg?;
            let text = msg
                .as_text()
                .ok_or_else(|| eyre!("Received non-text message from WebSocket: {:?}", msg))?;
            let resp = simd_json::serde::from_slice::<OkxWsDataResponse<OkxCandleData>>(
                &mut text.to_string().into_bytes(),
            )?;
...
impl DataSubscriber<OkxWsResponse<OkxBookData>> for OkxClientV5 {
    async fn subscribe_data(
        &mut self,
        params: <OkxWsResponse<OkxBookData> as DataResponse>::Request,
    ) -> Result<impl StreamExt<Item = <OkxWsResponse<OkxBookData> as RawData>::Data> + Send> {
        let (mut client, _) = tokio_websockets::client::Builder::new()
            .uri("wss://wspap.okx.com/ws/v5/public")?
            .connect()
            .await?;
        ...
        let _resp = if let Some(Ok(msg)) = client.next().await
            && let Some(text) = msg.as_text()
        {
            simd_json::serde::from_slice::<OkxWsResponse<OkxBookData>>(
                &mut text.to_string().into_bytes(),
            )?
        } else {
            bail!("Failed to subscribe to OKX WebSocket channel")
        ...
        let stream = DataStream::new(client, |msg| -> Result<_> {
            ...
            let text = msg
                .as_text()
                .ok_or_else(|| eyre!("Received non-text message from WebSocket: {:?}", msg))?;
            let resp = simd_json::serde::from_slice::<OkxWsDataResponse<OkxBookData>>(
                &mut text.to_string().into_bytes(),
            )
            .context(format!("Unexpected response {text}"))?;
...
mod test {
    ...
    async fn okx_websocket_subscribe_mock_test() -> Result<()> {
        ...
        std::thread::spawn(move || {
            handle.spawn(async move {
                while let Ok((stream, _)) = listener.accept().await {
                    ...
                    ws_stream
                        .send(Message::text(simd_json::to_string(&OkxWsResponse::<
                            OkxCandleData,
                        >::default(
                        ))?))
                        .await?;
                    ...
                    for _ in 0..TIMES {
                        ws_stream
                            .send(Message::text(simd_json::to_string(&OkxWsDataResponse::<
                                OkxCandleData,
                            >::default(
                            ))?))
                            .await?;
        ...
        // let params =
        //     OkxWsRequest::builder("subscribe", vec![OkxArg::new("candle1D", "BTC-USDT")]).build();
